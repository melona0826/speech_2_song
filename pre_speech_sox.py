...
try:
    ...
    if sys.version_info < (3, 8):
        print("[WARN] pickle5 미설치 – protocol 5 파일은 읽지 못합니다.")
...
def process(cfg: Dict[str, str]):
    ...
    vocals, _ = librosa.load(cfg["vocal_wav"], sr=sr, mono=True, dtype=np.float32)
    speech, _ = librosa.load(cfg["speech"],    sr=sr, mono=True, dtype=np.float32)
    mr,     _ = librosa.load(cfg["mr"],        sr=sr, mono=True, dtype=np.float32)
    print("Load End")
    ...
    for seg in segments:
        ...
        speech_st  = (s_frame * hop) if s_frame is not None else ptr
        ptr        = speech_st + seg_len

        raw_seg    = speech_all[speech_st:ptr]
...
if __name__ == "__main__":
    ...
    process(
        {
            "speech":      "speech.wav",
            "mr":          "./mr/see_you_again/mr.wav",
            "vocal_data":  "./vocal_data/see_you_again/vocal_data.pkl",
            "vocal_wav":   "./vocals/see_you_again/vocals.wav",
        }
    )
