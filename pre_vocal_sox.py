...
def preprocess_vocal(vocal_path, out_path):
    vocals, sr = librosa.load(vocal_path, sr=None)
    ...
    for start, end in vocal_nonsilent:
        ...
        f0_v, t_v = pw.harvest(seg, sr, frame_period=FRAME_PERIOD)
        segments.append({
            'start': int(start),
            'end':   int(end),
            'f0_v':   f0_v.astype(np.float64),
            't_v':    t_v.astype(np.float64)
        })
    mfcc_v = librosa.feature.mfcc(
        y=vocals, sr=sr, n_mfcc=13,
